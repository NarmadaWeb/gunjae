...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _loadData() async {
    ...
    final user = await repo.store.getUserById(repo.currentUserId!);
    if (user == null) {
      // User ID exists in session but not in DB (edge case)
      repo.logout();
      if (mounted) Navigator.pushReplacementNamed(context, '/login');
      return;
    }
          ...
          child: Column(
            children: [
              ...
              Row(
                ...
                children: [
                  _buildMenuButton(
                      context, "Favorit", Icons.favorite, Colors.red),
                  _buildMenuButton(
                      context, "Ulasan", Icons.reviews, Colors.amber),
                  _buildMenuButton(
                      context, "Voucher", Icons.local_offer, Colors.blue),
                ],
              ),
              const SizedBox(height: 32),
              ...
              if (_activeBookings.isNotEmpty)
                ...
                const Padding(
                  ...
                  child: Text("Tidak ada reservasi aktif.",
                ...
                children: [
                  ClipRRect(
                    ...
                    child: Image.network(spot.imageUrl,
                        height: 128, width: double.infinity, fit: BoxFit.cover),
                  ),
                  Positioned(
                    top: 12,
                    left: 12,
                          ...
                          child: ElevatedButton.icon(
                            onPressed: () async {
                              await Navigator.pushNamed(
                                  context, '/edit_booking', arguments: {
                                'booking': booking,
                                'spot': spot
                              });
                              _loadData();
